...
import os
import struct
from src.data.basetypes import ICVLFrame, NamedImgSequence
from src.util.handdetector import HandDetector
from src.data.transformations import transformPoint2D
import pickle
import matplotlib
...
class ICVLImporter(DepthImporter):
    ...
    def loadSequence(self, seqName, subSeq=None, Nmax=float('inf'), shuffle=False, rng=None, docom=False):
        ...
        """
        print('load data {}'.format(seqName))
        ...
        if self.useCache:
            # load data from cache when the subSeq length is 1
        ...
        for line in inputfile:
            ...
            if not hd.checkImage(1):
                ...
                continue
            try:
                dpt, M, com = hd.cropArea3D(gtorig[0],size=config['cube'],docom=False)
            except UserWarning:
