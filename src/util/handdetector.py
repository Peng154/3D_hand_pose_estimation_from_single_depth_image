...
class HandDetector(object):
    ...
    def bilinearResize(src, dsize, ndValue):
        ...
        x_ratio = float(shape[1] - 1) / dst.shape[1]
        y_ratio = float(shape[0] - 1) / dst.shape[0]
    ...
    def cropArea3D(self, com=None, size=(250, 250, 250), dsize=(128, 128), docom=False):
        ...
        #############
        # 下面就是把截取好的图像转换成128*128大小，包含了最终的变换矩阵
        wb = (xend - xstart)
        hb = (yend - ystart)
        # 保证按比例缩放。。。
        if wb > hb:
    ...
    def sampleRandomPoses(importer, rng, base_poses, base_com, base_cube, num_poses, aug_modes,
        ...
        off = rng.randn(int(num_poses), 3) * 7. # 偏移量
        sc = numpy.fabs(rng.randn(int(num_poses)) * 0.1 + 1.) # 缩放量
        rot = rng.uniform(0, 360, size=(int(num_poses), 3)) # 旋转量
