...
class DepthSenseCameraDevice(CameraDevice):
    ...
    def start(self):
        ...
        print("depth camera mssage:", self.dev.get_device_info())
    ...
    def getDepth(self):
        ...
        dframe_data = numpy.array(depth_frame.get_buffer_as_triplet()).reshape([480, 640, 2])
        dpt1 = numpy.asarray(dframe_data[:,:,0], dtype='float32')
        dpt2 = numpy.asarray(dframe_data[:,:,1], dtype='float32')
        dpt2 *= 255
        dpt = dpt1 + dpt2
        # print "get depth img"
        return True, dpt
